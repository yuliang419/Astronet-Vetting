...
def box_model(time, t0, duration, depth, mean_flux=1):
    """
    ...
    :param mean_flux: mean of the OOT flux.
    :return: 1D array of model flux values corresponding to input times.
    """
    model = np.ones(len(time)) * mean_flux
...
def find_secondary(time, flux, duration, period, mask_width=2, phase_limit=0.1):
    ...
    mean_flux = np.mean(new_flux)  # centre flux at zero
    new_flux -= mean_flux
    ...
    for t0 in time_grid:
        ...
        if SR > best_SR:
            ...
            best_SR = SR
            depth = abs(-s/(1-r) - s/r)
    return best_t0, new_time, new_flux+mean_flux, depth
