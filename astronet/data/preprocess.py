...
def read_and_process_light_curve(tic, tess_data_dir, sector=1, injected=False, inject_dir='/pdo/users/yuliang'):
  ...
  all_time, all_mag, mag_small, mag_big = tess_io.read_tess_light_curve(file_names)
  ...
  flux_small = 10. ** (-(mag_small - np.median(mag_small)) / 2.5)
  flux_big = 10. ** (-(mag_big - np.median(mag_big)) / 2.5)
  return all_time, all_flux, flux_small, flux_big
...
def generate_view(time, flux, num_bins, bin_width, t_min, t_max,
  ...
  if normalize:
    view -= np.median(view)
    if np.min(view) == 0:
        tf.logging.info("# points in light curve: %s" % (len(view)))
...
def find_secondary(time, flux, duration, period, mask_width=2, phase_limit=0.1):
    ...
    if len(new_time) < 5:
        tf.logging.info("Not enough points for secondary view.")
        raise SparseLightCurveError
    ...
    mean_flux = np.mean(new_flux)
    new_flux -= mean_flux  # flux should have arithmetic mean of zero
    ...
    for t0 in time_grid:
        ...
        if SR > best_SR:
            best_t0 = t0
            best_SR = SR
    return best_t0, new_time, new_flux+mean_flux
...
def measure_eclipse_depth(time, flux, duration, Qingress):
    """
    Given a phase-folded light curve, fit a box of fixed width to the centre of the light curve and return the depth.
    :param time: 1D array of time values, sorted in ascending order, with the transit located at time 0.
    :param flux: 1D array of flux values.
    :param duration: The duration of the event (in days).
    :param Qingress: Duration of ingress divided by total transit duration.
    :return:
    Depth of the transit event.
    """
    full_transit_duration = duration*(1 - 2*Qingress)
    in_transit = np.where(abs(time) < full_transit_duration/2)
    out_transit = np.where(abs(time) >= duration/2)

    if len(in_transit[0]) < 4:
        in_transit = np.where(abs(time) < duration / 2)
    if len(in_transit[0]) < 1:
        tf.logging.info("Bad transit duration: %s", duration)
        raise ValueError

    L = np.mean(flux[in_transit])
    H = np.mean(flux[out_transit])

    return H-L


def measure_odd_even(time, flux, duration, Qingress, period, t0):
    """
    Measure difference in depth between odd and even transits. For some reason the depths returned are not always
    reliable.
    :param time: Non-phase folded time
    :param flux:
    :param duration:
    :param Qingress:
    :param period:
    ...
    try:
        odd_time, odd_flux = phase_fold_and_sort_light_curve(time[odd], flux[odd], period, t0)
        even_time, even_flux = phase_fold_and_sort_light_curve(time[even], flux[even], period, t0)
    except ValueError:
        return 0

    odd_depth = measure_eclipse_depth(odd_time, odd_flux, duration, Qingress)
    even_depth = measure_eclipse_depth(even_time, even_flux, duration, Qingress)
